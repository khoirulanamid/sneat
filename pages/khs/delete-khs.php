...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($confirm === 'yes' && $id > 0) {
        try {
            $stmt = $pdo->prepare("DELETE FROM khs WHERE id_khs = :id");
            $stmt->execute([':id' => $id]);
...
$stmt = $pdo->prepare(
    "SELECT khs.id_khs, mahasiswa.nama, mahasiswa.nim, matakuliah.nama_matkul, matakuliah.kode_matkul
     FROM khs
...
<head>
    <meta charset="UTF-8">
    <title>Konfirmasi Hapus KHS</title>
...
<h4 class="fw-bold mb-3"><span class="text-muted fw-light">KHS /</span> Konfirmasi Hapus</h4>
    ...
    <div class="card delete-card">
        <div class="card-body">
            <div class="d-flex align-items-center mb-2">
                ...
                <h5 class="mb-0 text-white">Hapus KHS?</h5>
            </div>
