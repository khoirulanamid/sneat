...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($confirm === 'yes' && $id > 0) {
        try {
            $stmt = $pdo->prepare("DELETE FROM ktm WHERE id_ktm = :id");
            $stmt->execute([':id' => $id]);
...
$stmt = $pdo->prepare(
    "SELECT ktm.id_ktm, mahasiswa.nama, mahasiswa.nim, ktm.nomor_kartu
     FROM ktm
...
<head>
    <meta charset="UTF-8">
    <title>Konfirmasi Hapus KTM</title>
...
<h4 class="fw-bold mb-3"><span class="text-muted fw-light">KTM /</span> Konfirmasi Hapus</h4>
    ...
    <div class="card delete-card">
        <div class="card-body">
            <div class="d-flex align-items-center mb-2">
                ...
                <h5 class="mb-0 text-white">Hapus KTM?</h5>
            </div>
            ...
            <div class="p-3 mb-3 delete-meta text-white">
                ...
                <div><strong>Nomor KTM:</strong> <?php echo htmlspecialchars($row['nomor_kartu'] ?? '-'); ?></div>
            </div>
